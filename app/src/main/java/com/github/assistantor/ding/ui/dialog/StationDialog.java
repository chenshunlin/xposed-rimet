...
import com.github.assistantor.ding.contract.StationContract;
import com.github.assistantor.ding.data.model.StationModel;
import com.github.assistantor.ding.presenter.StationPresenter;
import com.github.assistantor.ding.ui.adapter.StationAdapter;
        ...
        if (1 == itemId) {
            ...
            if (null != models) {
                ...
                for (final JsonElement elem : array) {
                    StationModel tmpModel = new Gson().fromJson(elem, StationModel.class);
                    boolean isContain = false;
                    for (StationModel model : mStationModels) {
                        if (model.hashCode() == tmpModel.hashCode() || model.getName().equals(tmpModel.getName())) {
                            isContain = true;
                        }
                    }
                    if (!isContain) mStationModels.add(tmpModel);
                }
                mPresenter.save(mStationModels);
                ToastUtil.show("基站配置导入成功！");
            }
            return true;
        } else if (4 == itemId) {
            // 导出
            DialogUtil.showDialog(getContext(),
                    "提示", "\n是否导出基站配置？", (dialog, which) -> {
                        if (mStationModels != null) {
                            String jsonString = new Gson().toJson(mStationModels);
                            ClipboardManager cm = (ClipboardManager) getContext().getSystemService(Context.CLIPBOARD_SERVICE);
                            // 将文本内容放到系统剪贴板里。
                            cm.setText(jsonString);
                            ToastUtil.show("基站信息导出到粘贴板成功！");
                        }
                    });
            return true;
        }
