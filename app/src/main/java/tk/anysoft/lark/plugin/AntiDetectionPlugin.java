...
public class AntiDetectionPlugin extends BaseDingPlugin {
    ...
    public void hook() {
        ...
        //反检测 xp fakegps等 应该是获取列表hook 还有目标应用自身方法hook 返回通过
        //降级处理，获取版本这里尝试获取官方或者其他应用商店最新版本，让目标应用始终认为最新版打卡，或者设置指定版本号
                ...
                .after(param -> {
                    if (isEnable(XConstant.Key.ENABLE_ANTI_DETECTION)) {
                        if (isEnable(XConstant.Key.ENABLE_ANTI_DOWNGRADE)) {
                            if (XConstant.apps.PACKAGE_NAME.contains(param.args[0])) {
