...
public class AnalysisTask extends AbstractTask<String, String, Map<Integer, String>> {
    ...
    protected Map<Integer, String> onHandler(String... strings) throws Exception {
        ...
        for (String className : classNames) {
            ...
            if ((!packageName.equals(XConstant.apps.PACKAGE_NAME.get(1)) && mStringMap.size() >= 3) //rimet
                    || (packageName.equals(XConstant.apps.PACKAGE_NAME.get(1)) && mStringMap.size() >= 2)) {//ding lite
                // 不需要处理了
                Alog.d(">>>>>>>>>>>>>>> 不需要处理了");
                break;
            }
    ...
    private void handlerClass(String className, String packageName) {
        ...
        } else if (Object.class.equals(superClass) && !packageName.equals(XConstant.apps.PACKAGE_NAME.get(1)) && handlerRedPacketsClass(tClass)) {
